...
# 计算文本长度特征
train_lengths = np.array([len(tokens) for tokens in train_tokens]).reshape(-1, 1)
test_lengths = np.array([len(tokens) for tokens in test_tokens]).reshape(-1, 1)
...
train_negations = np.array([count_negations(tokens) for tokens in train_tokens]).reshape(-1, 1)
test_negations = np.array([count_negations(tokens) for tokens in test_tokens]).reshape(-1, 1)

# 合并特征
train_features = np.hstack([train_vectors, train_lengths, train_negations])
test_features = np.hstack([test_vectors, test_lengths, test_negations])
